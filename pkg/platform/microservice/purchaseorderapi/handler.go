...
func (s *Handler) Update(inputBytes []byte, applicationInfo platform.Application) (platform.HttpInputPurchaseOrderInfo, *Error) {
	// Function assumes access check has taken place
	var ms platform.HttpInputPurchaseOrderInfo
	logger := s.logContext.WithFields(logrus.Fields{
		"handler": "PurchaseOrderAPI",
	...
	return ms, s.ensureRawDataLogExists(msK8sInfo, ms, logger)
...
func (s *Handler) ensureRawDataLogExists(msK8sInfo k8s.MicroserviceK8sInfo, ms platform.HttpInputPurchaseOrderInfo, logger *logrus.Entry) *Error {
	...
	if !rawDataLogExists {
		...
		statusError := s.createRawDataLog(msK8sInfo, ms, logger)
		if err != nil {
			return statusError
		}
	} else {
		if err = s.rawdatalogRepo.Update(msK8sInfo.Namespace, msK8sInfo.Tenant, msK8sInfo.Application, s.extractRawDataLogInfo(ms)); err != nil {
