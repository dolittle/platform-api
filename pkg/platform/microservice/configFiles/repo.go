...
type ConfigFilesRepo interface {
	GetConfigFiles(applicationID string, environment string, microserviceID string) ([]platform.StudioConfigFile, error)
...
func (r k8sRepo) GetConfigFiles(applicationID string, environment string, microserviceID string) ([]platform.StudioConfigFile, error) {
	...
	for name, value := range configMap.Data {
		data = append(data, platform.StudioConfigFile{
			Name:     name,
			Value:    value,
		})
	}
